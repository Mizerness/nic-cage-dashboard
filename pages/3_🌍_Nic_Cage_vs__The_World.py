...
try:
    ...
    with col1:
        if cage_poster:
            # Create an empty container to center the button
            center_button = st.empty()
            if center_button.button("Vote for Nicolas Cage"):
                st.success("You made the right decision !")
    ...
    with col2:
        if actor_poster:
            center_button = st.empty()
            if center_button.button(f"Vote for {selected_actor}"):
                st.error(f"Sadly, {selected_actor} lost :(")
    ...
    st.markdown(f"<h3 style='text-align: center;'>Some statistics if you can't choose...</h3>", unsafe_allow_html=True)
